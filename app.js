...
var db;
DbConnect.connect(function(db1){//连接数据库成功了
    db=db1;
});
...
io.on('connection', function (socket) {//注册一个回调函数，客户端是先建立连接再注册用户的
 ...
 //   showOnline();
  ...
  // when the client emits 'add user', this listens and executes
  socket.on('get initial', function (username) {//登陆成功了，建立了连接后的   
    var db_util=new FriendUtil();
    db_util.setName(username);
    db_util.getFriends(db,"we_chat_friends",function(friends){
      db_util.getRequests(db,"we_chat_friends",function(requests){  
        msg_util.setName(username);      
        msg_util.getUnRead(db,"we_chat_messages",function(unread){ 
          socket.emit('initial list', {//这个应该是给用户自己的
            friendList: friends,
  ...
  socket.on('message to',function(data){//只是向某个人发送信息
    ...
    if(utils.ifLogin(to,usernames)){  
      ...
      msg_util.setName(to);
      msg_util.setFrom(from);
      msg_util.setMess(message);

      msg_util.update(db,"we_chat_messages");
